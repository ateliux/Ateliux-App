...
function mapStatus(status: NewsletterStatusDto): NewsletterSubscriberStatus {
  if (status === "ACTIVE") return "Ativo";
  if (status === "UNSUBSCRIBED") return "Descadastrado";
  return "Novo";
}

function toApiStatus(status: NewsletterSubscriberStatus): NewsletterStatusDto {
  if (status === "Ativo") return "ACTIVE";
  if (status === "Descadastrado") return "UNSUBSCRIBED";
  return "NEW";
}

function toSubscriber(item: NewsletterSubscriberDto, index: number): NewsletterSubscriber {
  return {
    id: index + 1,
    apiId: item.id,
    email: item.email,
    name: item.name ?? "Sem nome",
    origin: item.origin ?? "Site",
    status: mapStatus(item.status),
    createdAt: formatDate(item.createdAt),
    interests: item.interests.length ? item.interests : ["Produto"],
  };
}
...
export function NewsletterManagementView() {
  ...
  const loadSubscribers = useCallback(async () => {
    ...
    setError("");
    try {
      const response = await listNewsletterSubscribers();
      setSubscribers(response.map(toSubscriber));
      setSource("api");
    } catch (loadError) {
      if (canUseDevFallback("admin/newsletter-management")) {
        setSubscribers([...NEWSLETTER_SUBSCRIBERS]);
        setSource("mock");
      } else {
        setSubscribers([]);
        setError(loadError instanceof Error ? loadError.message : "Nao foi possivel carregar os assinantes.");
      }
    } finally {
      setLoading(false);
    }
  ...
  async function saveSubscriber() {
    try {
      const interests = draft.interests.length ? [...draft.interests] : ["Produto"];
      if (source === "api") {
        const created = await createNewsletterSubscriber({ email: draft.email, name: draft.name, origin: draft.origin, interests });
        setSubscribers((current) => [toSubscriber(created, 0), ...current]);
      } else {
        setSubscribers((current) => [{ ...draft, id: Date.now(), interests }, ...current]);
      }
      setCreateOpen(false);
      setDraft(emptyDraft);
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : "Nao foi possivel salvar o assinante.");
    }
  }

  async function updateStatus(subscriber: NewsletterSubscriber, status: NewsletterSubscriberStatus) {
    try {
      if (source === "api" && subscriber.apiId) {
        const updated = await updateNewsletterSubscriber(subscriber.apiId, { status: toApiStatus(status) });
        setSubscribers((current) => current.map((item) => item.id === subscriber.id ? toSubscriber(updated, subscriber.id - 1) : item));
      } else {
  ...
  async function removeSubscriber() {
    ...
    try {
      if (source === "api" && deleteSubscriber.apiId) await deleteNewsletterSubscriber(deleteSubscriber.apiId);
      setSubscribers((current) => current.filter((subscriber) => subscriber.id !== deleteSubscriber.id));
      setDeleteSubscriber(null);
    } catch (deleteError) {
      setError(deleteError instanceof Error ? deleteError.message : "Nao foi possivel remover o assinante.");
    }
  }

  async function exportCsv() {
    try {
      const exported = source === "api" ? await exportNewsletterSubscribers() : { csv: undefined };
      setNotice(`${filteredSubscribers.length} assinantes preparados para exportacao.${exported.csv ? " CSV retornado pela API." : ""}`);
    } catch (exportError) {
      setError(exportError instanceof Error ? exportError.message : "Nao foi possivel exportar.");
    }
  }
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          <AdminButton variant="secondary" onClick={() => void exportCsv()}><Download className="h-4 w-4" /> Exportar CSV</AdminButton>
      ...
      {error ? <ErrorState title="Falha na newsletter" description={error} onRetry={loadSubscribers} /> : null}
      ...
      <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        {loading ? <LoadingState title="Carregando assinantes" /> : filteredSubscribers.length === 0 ? <EmptyState title="Nenhum assinante encontrado" /> : (
            ...
            <table className="w-full min-w-[960px] text-left">
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredSubscribers.map((subscriber) => (
                  <tr key={subscriber.apiId ?? subscriber.id} className="hover:bg-gray-50/60">
                    ...
                    <td className="p-4">
                      <div className="flex justify-end gap-2">
                        <button type="button" onClick={() => void updateStatus(subscriber, "Ativo")} className="rounded-lg bg-gray-50 px-3 py-2 text-xs font-semibold text-gray-500 hover:text-[#00B074]">Ativar</button>
                        <button type="button" onClick={() => void updateStatus(subscriber, "Descadastrado")} className="rounded-lg bg-gray-50 px-3 py-2 text-xs font-semibold text-gray-500 hover:text-gray-900">Descadastrar</button>
      ...
      <Modal isOpen={isCreateOpen} onClose={() => setCreateOpen(false)} title="Novo assinante" description="Cadastro enviado para a API de newsletter.">
        <div className="grid gap-4">
          ...
          <div className="flex justify-end gap-3"><AdminButton variant="secondary" onClick={() => setCreateOpen(false)}>Cancelar</AdminButton><AdminButton onClick={() => void saveSubscriber()}>Salvar</AdminButton></div>
        </div>
      </Modal>

      <Modal isOpen={Boolean(deleteSubscriber)} onClose={() => setDeleteSubscriber(null)} title="Remover assinante">
        {deleteSubscriber ? <div className="space-y-5"><p className="text-sm text-gray-600">Confirma a remocao de <strong>{deleteSubscriber.email}</strong>?</p><div className="flex justify-end gap-3"><AdminButton variant="secondary" onClick={() => setDeleteSubscriber(null)}>Cancelar</AdminButton><AdminButton variant="danger" onClick={() => void removeSubscriber()}>Remover</AdminButton></div></div> : null}
      </Modal>
    </div>
  );
}
