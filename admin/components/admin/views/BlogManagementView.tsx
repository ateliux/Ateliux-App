...
type DraftPost = Omit<AdminBlogPost, "id">;
...
export function BlogManagementView() {
  const [posts, setPosts] = useState<AdminBlogPost[]>([...ADMIN_BLOG_POSTS]);
  ...
  function setPostStatus(postId: number, status: BlogPostStatus) {
    setPosts((current) => current.map((post) => (post.id === postId ? { ...post, status } : post)));
  }

  function duplicatePost(post: AdminBlogPost) {
    setPosts((current) => [
      {
        ...post,
        id: Date.now(),
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">CRUD visual dos artigos da Ateliux com rascunho, publicacao, agendamento, arquivo e previa.</p>
        </div>
                ...
                <tr key={post.id} className="hover:bg-gray-50/60">
                  ...
                  <td className="p-4">
                    <div className="flex justify-end gap-2">
                      ...
                      <button type="button" onClick={() => duplicatePost(post)} className="rounded-lg bg-gray-50 p-2 text-gray-500 hover:text-[#00B074]" aria-label="Duplicar artigo"><Copy className="h-4 w-4" /></button>
                      <button type="button" onClick={() => setPostStatus(post.id, post.status === "Publicado" ? "Rascunho" : "Publicado")} className="rounded-lg bg-gray-50 p-2 text-gray-500 hover:text-[#00B074]" aria-label={post.status === "Publicado" ? "Despublicar artigo" : "Publicar artigo"}><UploadCloud className="h-4 w-4" /></button>
                      <button type="button" onClick={() => setPostStatus(post.id, "Arquivado")} className="rounded-lg bg-gray-50 p-2 text-gray-500 hover:text-[#00B074]" aria-label="Arquivar artigo"><Archive className="h-4 w-4" /></button>
      ...
      <Modal isOpen={isEditorOpen} onClose={() => setEditorOpen(false)} title={editingPost ? "Editar artigo" : "Novo artigo"} description="Preencha os dados do artigo para o blog da Ateliux." size="lg">
        <div className="grid gap-4">
          <label className="grid gap-2 text-sm font-semibold text-gray-700">
            Titulo
            <input value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value, slug: createSlug(event.target.value) }))} className={inputClassName} />
          </label>
          ...
          <div className="flex justify-end gap-3"><AdminButton variant="secondary" onClick={() => setEditorOpen(false)}>Cancelar</AdminButton><AdminButton onClick={savePost}>{editingPost ? "Salvar alteracoes" : "Criar artigo"}</AdminButton></div>
        </div>
      </Modal>
      ...
      <Modal isOpen={Boolean(deletePost)} onClose={() => setDeletePost(null)} title="Excluir artigo" description="Esta acao remove o artigo apenas do estado local da demonstracao.">
        {deletePost ? (
          <div className="space-y-5">
            <p className="text-sm text-gray-600">Confirma a exclusao de <strong>{deletePost.title}</strong>?</p>
            <div className="flex justify-end gap-3"><AdminButton variant="secondary" onClick={() => setDeletePost(null)}>Cancelar</AdminButton><AdminButton variant="danger" onClick={removePost}>Excluir</AdminButton></div>
          </div>
        ) : null}
      </Modal>
    </div>
