...
export function ClientPreviewPage() {
  ...
  return <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8"><ClientPortalPageHeader eyebrow="Ambiente de revisao" title="Previa do projeto" description="Acesse as paginas liberadas e registre comentarios diretamente no contexto correto." /><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{previews.map((preview) => <ClientPortalCard key={preview.id} className="flex flex-col p-6"><div className="flex items-start justify-between"><span className="grid h-11 w-11 place-items-center rounded-xl bg-slate-100"><MonitorPlay className="h-5 w-5" /></span><ClientPortalBadge variant={statusVariant(preview.status)}>{statusLabel[preview.status]}</ClientPortalBadge></div><h2 className="mt-5 font-bold text-slate-900">{preview.page}</h2><p className="mt-1 text-xs text-slate-400">Atualizada em {preview.updatedAt}</p><div className="mt-4 min-h-20 rounded-xl bg-slate-50 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-400">Comentarios</p>{preview.comments.length ? preview.comments.map((comment) => <p key={comment} className="mt-2 text-xs leading-5 text-slate-600">{comment}</p>) : <p className="mt-2 text-xs text-slate-400">Nenhum comentario registrado.</p>}</div><div className="mt-auto flex flex-wrap gap-2 pt-5">{preview.url && preview.status !== "unavailable" ? <a href={preview.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-black px-4 py-2.5 text-xs font-semibold text-white">Abrir previa<ExternalLink className="h-3.5 w-3.5" /></a> : <ClientPortalButton disabled>Indisponivel</ClientPortalButton>}<ClientPortalButton variant="secondary" onClick={() => setCommenting(preview)}><MessageSquareText className="h-4 w-4" />Comentar</ClientPortalButton></div></ClientPortalCard>)}</div>{commenting ? <ClientPortalModal title={`Comentar: ${commenting.page}`} description="O comentario sera associado a esta pagina apenas no estado local." onClose={() => setCommenting(null)}><form onSubmit={comment} className="space-y-5"><ClientPortalTextarea name="comment" label="Comentario ou ajuste solicitado" rows={5} required /><div className="flex justify-end gap-3"><ClientPortalButton variant="secondary" onClick={() => setCommenting(null)}>Cancelar</ClientPortalButton><ClientPortalButton type="submit">Enviar comentario</ClientPortalButton></div></form></ClientPortalModal> : null}{toast ? <ClientPortalToast message={toast} /> : null}</div>;
}
