...
import { toClientHistoryItem } from "@/lib/client-portal/api-adapters";
...
export function ClientHistoryPage() {
  ...
  const loadHistory = useCallback(async () => {
    ...
    setError("");
    try {
      const response = await listClientHistory<Record<string, unknown>>();
      setHistory(response.map(toClientHistoryItem));
    } catch (loadError) {
      if (canUseDevFallback("frontend/client-history")) {
        setHistory(clientHistory.slice().reverse());
      } else {
        setHistory([]);
